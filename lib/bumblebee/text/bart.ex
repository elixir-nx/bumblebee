defmodule Bumblebee.Text.Bart do
  ...
  ## Inputs
    ...
    * `"input_ids"` - indices of input sequence tokens in the vocabulary
      with shape `{batch_size, seq_length}`

    * `"attention_mask"` - a mask indicating which tokens to attend to.
      This is used to ignore padding tokens, which are added when
      processing a batch of sequences with different length. By default
      no input tokens are masked
    ...
    * `"head_mask"` - a mask to nullify selected heads of the self-attention
      ...
      By default no heads are masked
    ...
    * `"decoder_input_ids"` - indices of decoder input sequence tokens
      in the vocabulary with shape `{batch_size, target_seq_length}`.
      If not present and `"input_ids"` is, it will be generated by
      shifting each token in `"input_ids"` to the right once

    * `"decoder_attention_mask"` - a mask indicating which decoder
      tokens to attend to. This is used to ignore padding tokens, which
      are added when processing a batch of sequences with different
      length. By default no input tokens are masked

    * `"decoder_position_ids"` - indices of positions of each decoder
      input sequence tokens in the position embeddings

    * `"decoder_head_mask"` - a mask to nullify selected heads of the
      self-attention blocks in the decoder with shape
      `{decoder_layers, decoder_attention_heads}`. By default no heads
      are masked

    * `"decoder_input_embeds"` - embedded representation of `"decoder_input_ids"`
      with shape `{batch_size, seq_len, hidden_size}`, which can be
    ...
    * `"encoder_last_hidden_state"` - last hidden state output from the
      encoder with shape `{batch_size, seq_len, hidden_size}`. Encoder
      last hidden state is used in cross-attention blocks in the decoder.
      If specified, the model will skip the encoding process and use this
      value directly for cross-attentions in the decoder

    * `"cross_attention_head_mask"` - a mask to nullify selected heads
      of the cross-attention blocks in the decoder with shape
      `{decoder_layers, decoder_attention_heads}`. By default no heads
      are masked

    * `"cache"` - a container with cached layer results used to speed
      up sequential decoding (autoregression). With cache, certain
      hidden states are taken from the cache, rather than recomputed
  ...
  defp position_embedding(position_ids, config, opts) do
    ...
    # Fort BART we need to offset the embeddings
    offset = 2
