defmodule Bumblebee.Text.SigLipText do
  ...
  @moduledoc """
  The SigLIP model for text encoding.
  ...
  defp core(inputs, spec) do
    ...
    # Take the last token for pooling (SigLIP pools from last position)
    last_token =
      Axon.nx(hidden_state, fn x ->
        seq_len = Nx.axis_size(x, 1)
        x |> Nx.slice_along_axis(seq_len - 1, 1, axis: 1) |> Nx.squeeze(axes: [1])
      end)

    pooled_state =
      Axon.dense(last_token, spec.hidden_size,
  ...
  defimpl Bumblebee.HuggingFace.Transformers.Config do
    def load(spec, %{"model_type" => "siglip", "text_config" => data}) do
      load(spec, data)
    end
