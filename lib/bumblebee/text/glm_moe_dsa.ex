defmodule Bumblebee.Text.GlmMoeDsa do
  ...
  options =
    [
      ...
      indexer_block_types: [
        default: nil,
        doc: """
        a list with the indexer type for each block, either `:full` for blocks that run their
        own indexer, or `:shared` for blocks that reuse the selection made by the preceding
        `:full` block. When `nil`, every block runs its own indexer
        """
      ],
      use_attention_bias: [
        default: false,
  ...
  @moduledoc """
  GLM MoE DSA model family, as used by GLM-5.
  ...
  ## Architectures
    ...
    * `:base` - plain GLM MoE DSA without any head on top

    * `:for_causal_language_modeling` - GLM MoE DSA with a language
      modeling head. The head returns logits for each token in the
      original sequence

    * `:for_sequence_classification` - GLM MoE DSA with a sequence
  ...
  defp decoder(
    ...
    Layers.Transformer.blocks(hidden_state,
      ...
      attention: fn block_idx ->
        &latent_attention(&1, &2, spec, position_ids, block_idx)
      end,
  ...
  defp latent_attention(hidden_state, opts, spec, position_ids, block_idx) do
    name = opts[:name]
    ...
    {sparse_attention_bias, attention_cache} =
      if block_indexer_type(spec, block_idx) == :full do
        indexer(
          hidden_state,
          query_latent,
          position_ids,
        ...
        # Blocks with a shared indexer reuse the selection made by the
        # preceding block, which we do not thread through the decoder.
        # Attending to all keys is a superset of the sparse selection and
        # it is exactly equivalent for sequences no longer than the
        # indexer top-k
        {Layers.none(), attention_cache}
      end
  ...
  defp block_indexer_type(spec, block_idx) do
    case spec.indexer_block_types do
      nil -> :full
      types -> Enum.at(types, block_idx, :full)
    end
  end
    ...
    def load(spec, data) do
      ...
      opts =
        convert!(data,
          ...
          indexer_block_types:
            {"indexer_types", optional(list(mapping(%{"full" => :full, "shared" => :shared})))},
